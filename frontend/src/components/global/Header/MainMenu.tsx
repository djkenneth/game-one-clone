...
export const MainMenu = () => {
  return (
    <div className="hidden bg-egg-white px-4 py-3 md:block">
      <Container>
        <NavigationMenu>
            ...
            {menus.map((item) => (
              <NavigationMenuItem key={item.title}>
                <Link to={`/products${item.href}`}>
                  <NavigationMenuTrigger className="text-[0.8rem] font-bold uppercase tracking-wider text-gray-950 hover:text-red-600">
                    {item.title}
                  </NavigationMenuTrigger>
                </Link>
                <NavigationMenuContent>
                  <ul className="grid grid-cols-4 gap-3 p-4 md:w-[500px] lg:w-[80rem]">
                    {item.subMenu.map((subMenuItem) => (
                      <li
                        key={subMenuItem.title}
                        className="p-3 text-[0.8rem] font-bold uppercase tracking-wider text-gray-950 hover:text-red-600"
                      >
